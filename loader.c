 ...
 * Version mit absoluter Pfad-Diagnose für armhf GL-Stack
 * ========================================================================== */
...
/* ==========================================================================
 * 10. GL-STACK-CHECK (absolute Pfade)
 * ========================================================================== */
static void check_gl_symbols(void) {
    ...
    const char *paths[] = {
        ...
        "/usr/local/lib/arm-linux-gnueabihf/libEGL.so.1.0.0",
    ...
    for (int i = 0; paths[i]; i++) {
        ...
        if (stat(paths[i], &st) == 0)
            rawf("[NFS]   EXISTIERT: %s (size=%ld)\n", paths[i], (long)st.st_size);
        else
            rawf("[NFS]   FEHLT:      %s (%s)\n", paths[i], strerror(errno));
    }

    /* 2. dlopen mit vollem Pfad versuchen */
    for (int i = 0; paths[i]; i++) {
        void *h = dlopen(paths[i], RTLD_NOW | RTLD_GLOBAL);
        if (h) rawf("[NFS]   dlopen(\"%s\") = %p OK\n", paths[i], h);
        else   rawf("[NFS]   dlopen(\"%s\") = FAIL: %s\n", paths[i], dlerror());
    }

    /* 3. Symbole im gesamten Prozess suchen */
    const char *symbols[] = {
...
int main(int argc, char **argv){
    ...
    SDL_Window *w = SDL_CreateWindow("NFS MW",
        ...
        SDL_WINDOW_SHOWN);
    if (!w) rawf("[NFS] Fenster (ohne GL): %s\n", SDL_GetError());
    else {
        rawmsg("[NFS] Fenster (ohne GL) OK – teste GL-Kontext\n");
    ...
    if (!w) { rawf("[NFS] Fenster (mit GL): %s\n", SDL_GetError()); SDL_Quit(); return 2; }
