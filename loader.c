 ...
 * Version mit Verzeichnis-Auflistung + Fehler-Preservation + Visibility-Fix
 * ========================================================================== */
...
/* ==========================================================================
 * 10. DIAGNOSE: GL-Dateien + Verzeichnis-Auflistung von libs/
 * ========================================================================== */
static void check_gl_files(void) {
    ...
    /* ============================================================
     * DIREKTE VERZEICHNIS-AUFLISTUNG von libs/
     * ============================================================ */
    ...
    DIR *d = opendir("libs");
    if (d) {
        rawmsg("[NFS]   (Auflistung via relativem Pfad 'libs')\n");
        struct dirent *e;
        ...
        d = opendir("/roms/ports/nfs_mw/libs");
        if (d) {
            rawmsg("[NFS]   (Auflistung via absolutem Pfad)\n");
            struct dirent *e;
            while ((e = readdir(d)) != NULL) {
                struct stat st;
                char full[512];
                snprintf(full, sizeof(full), "/roms/ports/nfs_mw/libs/%s", e->d_name);
                if (stat(full, &st) == 0)
                    rawf("[NFS]     %-40s size=%-12ld mode=%o\n",
                         e->d_name, (long)st.st_size, st.st_mode & 07777);
                else
            ...
            rawf("[NFS]   opendir(\"/roms/ports/nfs_mw/libs\") FAIL: %s\n", strerror(errno));
        }
    }

    rawmsg("[NFS] --- Ende Verzeichnis-Auflistung ---\n");
    rawmsg("[NFS] --- Ende GL-Datei-Check ---\n");
}

/* ==========================================================================
 * 11. DLOPEN-HELFER (mit Fehler-Preservation)
 * ========================================================================== */
