 ...
 * Diagnose-Version mit unbuffered I/O + Crash-Handler + SDL-Subsystem-Diagnose
 * ========================================================================== */
...
int __android_log_print(int prio, const char *tag, const char *fmt, ...) {
    ...
    ssize_t r = write(1, buf, (size_t)total);
    (void)r;
    return 0;
}
...
void __cxa_throw(void *e, void *t, void *d){
    ...
    rawmsg("[NFS] C++ Exception geworfen (Mock)\n");
    abort();
}
...
int main(int argc, char **argv){
     ...
     * ============================================================ */
    {
        ...
        const char *ad = SDL_getenv("SDL_AUDIODRIVER");
        rawf("[NFS] SDL_AUDIODRIVER = %s\n", ad ? ad : "(nicht gesetzt)");
    ...
    rawmsg("[NFS] >>> SDL_GL_CreateContext...\n");
    ...
    if (!so){
        rawf("[NFS] FATAL: libNFSMW.so nicht ladbar: %s\n", dlerror());
    ...
    if (onload){
        ...
        rawmsg("[NFS] Kein JNI_OnLoad vorhanden (ok).\n");
    }
