 ...
 * Version mit ELF-Header-Analyse für .so-Dateien
 * ========================================================================== */
...
static void *g_java_vm_handle __attribute__((unused)) = (void*)g_vm_vtbl;
 ...
 * Alle mit visibility("hidden"), damit dlopen'd Bibliotheken (libNFSMW.so!)
 * NICHT unsere kaputten Stubs auflösen.
 * ========================================================================== */
#define HIDDEN __attribute__((visibility("hidden")))

HIDDEN void _ZNSt9exceptionD2Ev(void *p){ (void)p; }
HIDDEN void _ZNSt9exceptionD1Ev(void *p){ (void)p; }
HIDDEN void _ZNSt9exceptionD0Ev(void *p){ (void)p; }
HIDDEN void _ZNSt13runtime_errorD2Ev(void *p){ (void)p; }
HIDDEN void _ZNSt13runtime_errorD1Ev(void *p){ (void)p; }
HIDDEN void _ZNSt9bad_allocD2Ev(void *p){ (void)p; }
HIDDEN void _ZNSt9bad_allocD1Ev(void *p){ (void)p; }
HIDDEN void _ZNSt9type_infoD2Ev(void *p){ (void)p; }
HIDDEN void _ZNSt9type_infoD1Ev(void *p){ (void)p; }
HIDDEN void _ZSt9terminatev(void){ rawmsg("[NFS] std::terminate\n"); _exit(1); }
HIDDEN void _ZSt10unexpectedv(void){ rawmsg("[NFS] std::unexpected\n"); _exit(1); }
HIDDEN void __cxa_pure_virtual(void){ rawmsg("[NFS] pure virtual call\n"); _exit(1); }
HIDDEN void __cxa_deleted_virtual(void){ rawmsg("[NFS] deleted virtual call\n"); _exit(1); }
HIDDEN void *__cxa_allocate_exception(size_t n){ return calloc(1, n?n:1); }
HIDDEN void __cxa_free_exception(void *p){ free(p); }
HIDDEN void __cxa_throw(void *e, void *t, void *d){ (void)e;(void)t;(void)d; rawmsg("[NFS] C++ Exception (Mock)\n"); abort(); }
HIDDEN void *__cxa_begin_catch(void *e){ return e; }
HIDDEN void __cxa_end_catch(void){ }
HIDDEN void *__cxa_get_exception_ptr(void *e){ return e; }
HIDDEN void __cxa_rethrow(void){ abort(); }
HIDDEN void *__gxx_personality_v0(void){ return NULL; }
...
/* ==========================================================================
 * 10. ELF-HEADER-ANALYSE
 * Liest die DT_NEEDED / SONAME / RPATH / RUNPATH Einträge direkt aus der
 * ELF-Datei, um zu sehen, welche Abhängigkeiten sie verlangt.
 * ========================================================================== */
static void dump_elf_needs(const char *path) {
    rawf("[NFS] === ELF-Analyse: %s ===\n", path);
    FILE *f = fopen(path, "rb");
    if (!f) { rawf("[NFS]   fopen FAIL: %s\n", strerror(errno)); return; }
    ...
    int is_le = (ident[5] == 1);
    int osabi  = ident[7];
    rawf("[NFS]   Klasse: %s\n", is_64 ? "64-bit (ELFCLASS64)" : "32-bit (ELFCLASS32)");
    rawf("[NFS]   Endianness: %s\n", is_le ? "little" : "big");
    rawf("[NFS]   OSABI: %d %s\n", osabi,
         osabi == 0 ? "(System V)" :
         osabi == 3 ? "(GNU/Linux)" :
         osabi == 9 ? "(FreeBSD)" : "(?)");
    ...
    if (is_64) {
        rawmsg("[NFS]   -> 64-bit, NICHT in armhf-Prozess ladbar\n");
        fclose(f); return;
    }

    uint16_t e_machine, e_phentsize, e_phnum;
    uint32_t e_phoff;

    fseek(f, 18, SEEK_SET); fread(&e_machine, 2, 1, f);
    fseek(f, 28, SEEK_SET); fread(&e_phoff, 4, 1, f);
    fseek(f, 42, SEEK_SET); fread(&e_phentsize, 2, 1, f);
    fseek(f, 44, SEEK_SET); fread(&e_phnum, 2, 1, f);

    rawf("[NFS]   e_machine: %u %s\n", e_machine,
         e_machine == 40 ? "(ARM)" :
         e_machine == 3  ? "(x86)" :
         e_machine == 183 ? "(AArch64)" : "(?)");
    rawf("[NFS]   e_phoff=%u e_phnum=%u e_phentsize=%u\n",
         e_phoff, e_phnum, e_phentsize);

    if (e_machine != 40) {
        rawmsg("[NFS]   -> Falsche Architektur! Erwartet ARM (40).\n");
        fclose(f); return;
    }

    uint32_t dyn_off = 0, dyn_size = 0;
    struct { uint32_t vaddr, offset, filesz; } loads[16];
    int n_loads = 0;

    for (int i = 0; i < e_phnum && i < 32; i++) {
        long base = (long)e_phoff + (long)i * e_phentsize;
        uint32_t p_type, p_offset, p_vaddr, p_filesz;

        fseek(f, base + 0,  SEEK_SET); fread(&p_type,   4, 1, f);
        fseek(f, base + 4,  SEEK_SET); fread(&p_offset, 4, 1, f);
        fseek(f, base + 8,  SEEK_SET); fread(&p_vaddr,  4, 1, f);
        fseek(f, base + 16, SEEK_SET); fread(&p_filesz, 4, 1, f);

        if (p_type == 1 && n_loads < 16) {
            loads[n_loads].vaddr  = p_vaddr;
            loads[n_loads].offset = p_offset;
            loads[n_loads].filesz = p_filesz;
            n_loads++;
        }
        if (p_type == 2) {
            dyn_off  = p_offset;
    ...
    if (!dyn_off) { rawmsg("[NFS]   Kein PT_DYNAMIC gefunden\n"); fclose(f); return; }
    rawf("[NFS]   PT_DYNAMIC @ offset %u size %u\n", dyn_off, dyn_size);

    uint32_t dyn_count = dyn_size / 8;
    uint32_t strtab_vaddr = 0;
    struct { uint32_t tag, val; } entries[256];
    int n_entries = 0;

    fseek(f, dyn_off, SEEK_SET);
    for (uint32_t i = 0; i < dyn_count && n_entries < 256; i++) {
        uint32_t tag, val;
        if (fread(&tag, 4, 1, f) != 1) break;
        if (fread(&val, 4, 1, f) != 1) break;
        if (tag == 0) break;
        if (tag == 5) strtab_vaddr = val;
        if (tag == 1 || tag == 14 || tag == 15 || tag == 29) {
    ...
    if (strtab_vaddr) {
        for (int i = 0; i < n_loads; i++) {
            if (strtab_vaddr >= loads[i].vaddr &&
                strtab_vaddr <  loads[i].vaddr + loads[i].filesz) {
                strtab_off = (long)loads[i].offset +
                             (long)(strtab_vaddr - loads[i].vaddr);
                break;
            }
        }
    }
    rawf("[NFS]   DT_STRTAB vaddr=0x%x file_off=%ld\n", strtab_vaddr, strtab_off);

    rawmsg("[NFS]   DT_NEEDED / SONAME / RPATH / RUNPATH:\n");
    for (int i = 0; i < n_entries; i++) {
        const char *name = "?";
        switch (entries[i].tag) {
            case 1:  name = "NEEDED ";  break;
            case 14: name = "SONAME ";  break;
            case 15: name = "RPATH  ";  break;
            case 29: name = "RUNPATH"; break;
        }
        if (strtab_off < 0) {
            rawf("[NFS]     %s: (strtab unbekannt, vaddr+%u)\n", name, entries[i].val);
            continue;
        }
        fseek(f, strtab_off + entries[i].val, SEEK_SET);
        char needle[256] = {0};
        size_t got = fread(needle, 1, sizeof(needle) - 1, f);
        if (got > 0) rawf("[NFS]     %s: %s\n", name, needle);
        else         rawf("[NFS]     %s: (nicht lesbar)\n", name);
    }

    fclose(f);
    rawmsg("[NFS] === Ende ELF-Analyse ===\n");
}

/* ==========================================================================
 * 11. GL-DATEI-CHECK + Verzeichnis-Auflistung libs/
 * ========================================================================== */
static void check_gl_files(void) {
    rawmsg("[NFS] --- GL-Datei-Check (nur stat) ---\n");

    const char *paths[] = {
        "/usr/local/lib/arm-linux-gnueabihf/libEGL.so.1",
        "/usr/local/lib/arm-linux-gnueabihf/libGLESv2.so.2",
        "/usr/local/lib/arm-linux-gnueabihf/libgbm.so.1",
        "/usr/local/lib/arm-linux-gnueabihf/libmali-bifrost-g31-rxp0-gbm.so",
        "/dev/mali0",
        "/dev/dri/card0",
        "/dev/dri/renderD128",
        NULL
    ...
    for (int i = 0; paths[i]; i++) {
        ...
        if (stat(paths[i], &st) == 0)
            rawf("[NFS]   EXISTIERT: %s (mode=%o size=%ld)\n",
                 paths[i], st.st_mode & 07777, (long)st.st_size);
        else
            rawf("[NFS]   FEHLT:      %s (%s)\n", paths[i], strerror(errno));
    }

    rawmsg("[NFS] --- Verzeichnis-Auflistung libs/ ---\n");
    DIR *d = opendir("libs");
    if (d) {
        struct dirent *e;
        while ((e = readdir(d)) != NULL) {
            struct stat st;
            char full[512];
            snprintf(full, sizeof(full), "libs/%s", e->d_name);
            if (stat(full, &st) == 0)
                rawf("[NFS]     %-40s size=%-12ld mode=%o\n",
                     e->d_name, (long)st.st_size, st.st_mode & 07777);
        }
        closedir(d);
    } else {
        rawf("[NFS]   opendir(\"libs\") FAIL: %s\n", strerror(errno));
    }
    rawmsg("[NFS] --- Ende GL-Datei-Check ---\n");
}

/* ==========================================================================
 * 12. DLOPEN-HELFER (mit Fehler-Preservation)
 * ========================================================================== */
static char g_last_dl_error[512] = {0};
...
static void *try_load_so(const char *name){
    ...
    for (size_t i = 0; i < sizeof(dirs)/sizeof(dirs[0]); i++){
        ...
        rawf("[NFS]   dlopen(\"%s\", RTLD_LAZY|RTLD_GLOBAL)...\n", p);
        h = dlopen(p, RTLD_LAZY | RTLD_GLOBAL);
        if (h){ rawf("[NFS]   -> OK @ %p\n", h); return h; }
        const char *e = dlerror();
        if (e) {
            strncpy(g_last_dl_error, e, sizeof(g_last_dl_error) - 1);
            g_last_dl_error[sizeof(g_last_dl_error) - 1] = 0;
        }
        rawf("[NFS]   -> FAIL: %s\n", e ? e : "?");
    }
...
int main(int argc, char **argv){
    ...
    /* ELF-Analyse der Spieldateien */
    dump_elf_needs("libs/libNFSMW.so");
    dump_elf_needs("libs/libc++_shared.so");
    dump_elf_needs("libs/libfmodex.so");
    dump_elf_needs("libs/libfmodevent.so");
    dump_elf_needs("libs/libNimble.so");
    ...
    rawmsg("[NFS] >>> SDL_CreateWindow...\n");
    SDL_Window *w = SDL_CreateWindow("NFS MW",
        SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480,
        SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
    if (!w) { rawf("[NFS] Fenster: %s\n", SDL_GetError()); SDL_Quit(); return 2; }
    rawmsg("[NFS] Fenster OK\n");

    SDL_GLContext gl = SDL_GL_CreateContext(w);
    if (!gl) { rawf("[NFS] GL-Kontext: %s\n", SDL_GetError()); SDL_DestroyWindow(w); SDL_Quit(); return 3; }
    ...
    if (!so){
        rawf("[NFS] FATAL: libNFSMW.so nicht ladbar\n");
        rawf("[NFS] Letzter dlopen-Fehler: %s\n", g_last_dl_error);
        rawf("[NFS] Datei vorhanden? Prüfen: %s/libs/libNFSMW.so\n", cwd);
