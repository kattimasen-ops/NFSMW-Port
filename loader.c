 ...
 * Vollständige, kompilierbare loader.c
 * ========================================================================== */
...
/* ==========================================================================
 * 2. JNI-TYPEN (vollständig, inkl. JNIEnv / JavaVM)
 * ========================================================================== */
...
/* ==========================================================================
 * 5. VTABLE-ARRAYS (JNI-Standard-Layout)
 * ========================================================================== */
...
/* _JNIEnv / _JavaVM Handle (Zeiger auf Vtable) */
static void *g_jni_env_handle = (void*)g_jni_vtbl;
...
void _ZSt9terminatev(void){ fprintf(stderr,"[NFS] std::terminate\n"); exit(1); }
void _ZSt10unexpectedv(void){ fprintf(stderr,"[NFS] std::unexpected\n"); exit(1); }
void __cxa_pure_virtual(void){ fprintf(stderr,"[NFS] pure virtual call\n"); exit(1); }
void __cxa_deleted_virtual(void){ fprintf(stderr,"[NFS] deleted virtual call\n"); exit(1); }
void *__cxa_allocate_exception(size_t n){ return calloc(1, n?n:1); }
void __cxa_free_exception(void *p){ free(p); }
void __cxa_throw(void *e, void *t, void *d){
    (void)e;(void)t;(void)d;
    fprintf(stderr,"[NFS] C++ Exception geworfen (Mock)\n");
    abort();
}
...
/* ==========================================================================
 * 9. GAME-SPEZIFISCHE JNI-NATIVES (MogaController / NativeActivity / Renderer)
 * ========================================================================== */
...
int main(int argc, char **argv){
    ...
    if (!w){
        fprintf(stderr,"[NFS] Fenster: %s\n", SDL_GetError());
        SDL_Quit(); return 1;
    }
    SDL_GLContext gl = SDL_GL_CreateContext(w);
    if (!gl){
        fprintf(stderr,"[NFS] GL-Kontext: %s\n", SDL_GetError());
    ...
    if (!so){
        fprintf(stderr,"[NFS] FATAL: libNFSMW.so nicht ladbar: %s\n", dlerror());
